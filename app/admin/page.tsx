...
import { 
  ...
  Filter, ChevronDown as ChevDown, FileDown, Loader2
} from 'lucide-react';
...
const ROLE_FILTER_OPTIONS: { key: RoleFilter; label: string; color: string; bg: string; border: string }[] = [
  { key: 'all',                       label: 'Semua',       color: 'text-slate-700',   bg: 'bg-slate-600',   border: 'border-slate-600' },
  { key: UserRole.SISWA,              label: 'Siswa',       color: 'text-blue-700',    bg: 'bg-blue-600',    border: 'border-blue-600' },
  { key: UserRole.WALI_KELAS,         label: 'Wali Kelas',  color: 'text-purple-700',  bg: 'bg-purple-600',  border: 'border-purple-600' },
  { key: UserRole.GURU_PIKET,         label: 'Guru Piket',  color: 'text-amber-700',   bg: 'bg-amber-500',   border: 'border-amber-500' },
  { key: UserRole.SECURITY,           label: 'Security',    color: 'text-emerald-700', bg: 'bg-emerald-600', border: 'border-emerald-600' },
  { key: UserRole.ADMIN,              label: 'Admin',       color: 'text-red-700',     bg: 'bg-red-600',     border: 'border-red-600' },
];
...
export default function AdminPage() {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => processFile(e.target.files?.[0]);
  ...
  const downloadTemplate = async (role: UserRole) => {
    ...
    try {
      const res = await fetch(`/api/template?role=${role}`);
      if (!res.ok) throw new Error('Gagal mengunduh template');
      const blob = await res.blob();
      const disposition = res.headers.get('Content-Disposition') ?? '';
      const match = disposition.match(/filename="?([^"]+)"?/);
      const filename = match?.[1] ?? `template_${role}.csv`;
    ...
    MOCK_EXCEL_DATA.forEach(row => {
      ...
      if (!exists) {
        const payload: Omit<User, 'id'> = {
          ...
          role: importRole,
          email: row.email,
          ...(importRole === UserRole.SISWA && { nis: row.nis, kelas: row.kelas }),
          ...(importRole === UserRole.WALI_KELAS && { nip: row.nis, kelas: row.kelas }),
          ...(importRole !== UserRole.SISWA && importRole !== UserRole.WALI_KELAS && { nip: row.nis }),
        };
        addUser(payload);
        imported++;
      }
    });
    showToast(`Sukses mengimpor ${imported} pengguna sebagai ${ROLE_LABELS[importRole]}.`, 'success');
    setFileName(null); setFileSize(null); setShowPreview(false);
  ...
  // ──────────────────────────────────────────────
  const handleExportCSV = (exportRole: RoleFilter = 'all') => {
    const target = exportRole === 'all' ? users : users.filter(u => u.role === exportRole);
    ...
    const suffix = exportRole === 'all' ? 'semua' : exportRole;
    link.setAttribute('download', `pengguna_${suffix}_${new Date().toISOString().split('T')[0]}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    const label = exportRole === 'all' ? 'Semua Pengguna' : ROLE_LABELS[exportRole as UserRole];
  ...
  const openEditModal = (user: User) => {
    ...
    setFormName(user.name); setFormUsername(user.username); setFormPassword('');
    setFormRole(user.role); setFormEmail(user.email);
    setFormNis(user.nis || ''); setFormNip(user.nip || ''); setFormKelas(user.kelas || '');
    setFormErrors({});
  ...
  const handleEditSubmit = (e: React.FormEvent) => {
    ...
    updateUser(selectedUser.id, {
      name: formName, username: formUsername, role: formRole, email: formEmail,
  ...
  const handleDeleteExecute = () => {
    ...
    setIsDeleteModalOpen(false); setSelectedUser(null);
  };

  const closeAddModal = () => {
    setIsAddModalOpen(false);
    setFormName(''); setFormUsername(''); setFormPassword('');
    setFormRole(UserRole.SISWA); setFormEmail('');
    setFormNis(''); setFormNip(''); setFormKelas(''); setFormErrors({});
  };
  ...
  // Role stats
  const roleCounts = Object.values(UserRole).reduce((acc, r) => {
    acc[r] = users.filter(u => u.role === r).length;
    return acc;
  }, {} as Record<UserRole, number>);

  // eslint-disable-next-line react-hooks/rules-of-hooks
  useEffect(() => { setCurrentPage(1); }, [searchQuery, pageSize, roleFilter]);
    ...
    <AppShell>
          ...
          {tab === 'users' && (
            <>
              ...
              <div className="relative" ref={exportDropdownRef}>
                ...
                >
                  <Download size={14} /> Ekspor CSV <ChevDown size={12} className={`transition-transform ${exportDropdownOpen ? 'rotate-180' : ''}`} />
                </button>
                {exportDropdownOpen && (
                  <div className="absolute right-0 top-full mt-1 w-52 bg-white border border-slate-200 rounded-2xl shadow-xl z-20 overflow-hidden animate-fadeIn">
                    <div className="px-3 py-2 border-b border-slate-100">
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Ekspor berdasarkan peran</p>
                    </div>
                    <button
                    ...
                    {Object.values(UserRole).map(r => (
                      ...
                      >
                        <span className={`w-2 h-2 rounded-full flex-shrink-0 ${
                          { siswa:'bg-blue-500', wali_kelas:'bg-purple-500', guru_piket:'bg-amber-500', security:'bg-emerald-500', admin:'bg-red-500' }[r]
                        }`} />
        ...
        <div className="max-w-4xl space-y-6">
          <div className="card p-8 border-slate-200">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h3 className="font-bold text-slate-800 text-sm">Import Excel Data Pengguna</h3>
                <p className="text-xs text-slate-400 font-medium">Unggah data pengguna dalam format .xlsx / .xls / .csv untuk diimpor secara massal.</p>
              </div>
            </div>
            ...
            <div className="mb-5 p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-3">
              <div className="flex items-center justify-between gap-2 flex-wrap">
                ...
                >
                  {isDownloadingTemplate
                    ? <><Loader2 size={12} className="animate-spin" /> Mengunduh...</>
                    : <><FileDown size={12} /> Unduh Template {ROLE_LABELS[importRole]}</>
                  }
                </button>
              </div>
              ...
              <p className="text-[10px] text-slate-400 font-semibold">
                Semua baris data akan diimpor sebagai peran: <strong className={ROLE_FILTER_OPTIONS.find(o=>o.key===importRole)?.color || ''}>{ROLE_LABELS[importRole]}</strong>
            ...
            >
              ...
              {fileName ? (
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-350 mt-4 uppercase tracking-widest font-bold">Mendukung: .xlsx, .xls, .csv</p>
                </div>
              )}
            </div>
              ...
              <div className="p-5 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row gap-3 items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 font-semibold">
                      {MOCK_EXCEL_DATA.length} baris · akan diimpor sebagai <strong>{ROLE_LABELS[importRole]}</strong>
                    </p>
                  </div>
                </div>
                <button
      ...
      {tab === 'users' && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            {[
              { label: 'Total', count: users.length, color: 'from-slate-600 to-slate-700' },
              { label: 'Siswa', count: roleCounts[UserRole.SISWA] || 0, color: 'from-blue-500 to-blue-600' },
              { label: 'Wali Kelas', count: roleCounts[UserRole.WALI_KELAS] || 0, color: 'from-purple-500 to-purple-600' },
              { label: 'Guru Piket', count: roleCounts[UserRole.GURU_PIKET] || 0, color: 'from-amber-500 to-amber-600' },
              { label: 'Security', count: roleCounts[UserRole.SECURITY] || 0, color: 'from-emerald-500 to-emerald-600' },
              { label: 'Admin', count: roleCounts[UserRole.ADMIN] || 0, color: 'from-red-500 to-red-600' },
            ].map(stat => (
              <div key={stat.label} className={`bg-gradient-to-br ${stat.color} rounded-2xl p-3 text-white shadow-sm`}>
          ...
          <div className="card border-slate-200 overflow-hidden shadow-sm">
            ...
            <div className="px-5 py-3.5 border-b border-slate-100 flex flex-col sm:flex-row gap-3 items-center">
              ...
              <p className="text-xs text-slate-400 font-semibold font-mono whitespace-nowrap ml-auto">
                ...
                {roleFilter !== 'all' && ` · ${ROLE_LABELS[roleFilter as UserRole]}`}
              </p>
            </div>
                    ...
                    paginatedUsers.map(u => (
                      <tr key={u.id} className="hover:bg-slate-50/50 transition-colors group">
                        ...
                        <td className="px-5 py-3.5 text-xs font-bold text-slate-600">
                          {u.role === UserRole.SISWA && `${u.kelas || '-'} · NIS: ${u.nis || '-'}`}
                        ...
                        <div className="flex flex-col items-center gap-3 text-slate-400">
                          ...
                          <div>
                            ...
                            {roleFilter !== 'all' && (
                              ...
                              >
                                + Tambah {ROLE_LABELS[roleFilter as UserRole]} sekarang →
                              </button>
                            )}
                          </div>
                        </div>
              ...
              <div className="p-4 bg-slate-50/50 border-t border-slate-100 flex flex-col sm:flex-row gap-3 items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-slate-400 font-semibold font-mono ml-2">
                    {Math.min(sortedUsers.length, (currentPage - 1) * pageSize + 1)}–{Math.min(sortedUsers.length, currentPage * pageSize)} dari {sortedUsers.length}
                  </span>
                </div>
                <div className="flex items-center gap-1">
                  ...
                  {[...Array(Math.min(totalPages, 7))].map((_, i) => {
                    const page = totalPages <= 7 ? i + 1 : i === 0 ? 1 : i === 6 ? totalPages : currentPage - 2 + i;
                    return (
                      <button
                        key={i}
      ...
      {isAddModalOpen && (
        <UserFormModal
          ...
          formRole={formRole} setFormRole={r => { setFormRole(r); setFormErrors({}); }}
          formEmail={formEmail} setFormEmail={setFormEmail}
          formNis={formNis} setFormNis={setFormNis}
          formNip={formNip} setFormNip={setFormNip}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-3xl border border-slate-150 shadow-2xl w-full max-w-md z-10 p-6 space-y-4">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              Anda akan menghapus akun <strong>{selectedUser.name}</strong> ({ROLE_LABELS[selectedUser.role]}) dengan username <strong>{selectedUser.username}</strong>.
            </p>
            <div className="flex gap-2.5 justify-end pt-2 border-t border-slate-100">
              <button onClick={() => setIsDeleteModalOpen(false)} className="px-4 py-2 rounded-xl border border-slate-200 bg-white text-xs font-bold text-slate-500 hover:bg-slate-50">
                Batal
              </button>
              <button onClick={handleDeleteExecute} className="px-4 py-2 rounded-xl bg-red-600 hover:bg-red-700 shadow-md shadow-red-500/10 text-white font-bold text-xs">
                Hapus Sekarang
              </button>
            </div>
          </div>
...
function UserFormModal({
  ...
  formNis: string; setFormNis: (v: string) => void;
  formNip: string; setFormNip: (v: string) => void;
  formKelas: string; setFormKelas: (v: string) => void;
  ...
  const Field = ({ label, error, children }: { label: string; error?: string; children: React.ReactNode }) => (
    <div>
      <label className="label text-[10px] uppercase font-bold text-slate-400 tracking-wider">{label}</label>
      {children}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-2xl w-full max-w-lg z-10 overflow-hidden flex flex-col animate-slideUp">
        <div className="flex items-center justify-between p-5 border-b border-slate-100">
          ...
          <button onClick={onClose} className="p-1 rounded-lg hover:bg-slate-100 transition-colors text-slate-400">
            <X size={16} />
          </button>
        </div>

        <form onSubmit={onSubmit} className="p-5 space-y-4 overflow-y-auto max-h-[75vh]">
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              <Field label="Nama Lengkap" error={formErrors.name}>
                <input type="text" placeholder="Contoh: Muhammad Rafli" value={formName} onChange={e => setFormName(e.target.value)} className="input text-xs py-2" />
              </Field>
            </div>

            <Field label="Username / ID Login" error={formErrors.username}>
              <input type="text" placeholder="Contoh: rafli123" value={formUsername} onChange={e => setFormUsername(e.target.value)} disabled={isAdminUser} className="input text-xs py-2 font-mono" />
            </Field>

            <Field label={isEdit ? 'Sandi Baru (Kosongkan jika tidak diubah)' : 'Kata Sandi'} error={formErrors.password}>
              <input type="password" placeholder={isEdit ? 'Sandi baru...' : 'Masukkan sandi...'} value={formPassword} onChange={e => setFormPassword(e.target.value)} className="input text-xs py-2 font-mono" />
            </Field>

            <div className="col-span-2">
              <Field label="Alamat Email" error={formErrors.email}>
                <input type="email" placeholder="Contoh: siswa@sekolah.id" value={formEmail} onChange={e => setFormEmail(e.target.value)} className="input text-xs py-2" />
              </Field>
            </div>
              ...
              <div className="flex flex-wrap gap-2 mt-1">
                {Object.values(UserRole).map(r => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all border ${
                      formRole === r
                        ? { siswa:'bg-blue-100 border-blue-400 text-blue-700', wali_kelas:'bg-purple-100 border-purple-400 text-purple-700', guru_piket:'bg-amber-100 border-amber-400 text-amber-700', security:'bg-emerald-100 border-emerald-400 text-emerald-700', admin:'bg-red-100 border-red-400 text-red-700' }[r]
                        : 'bg-white border-slate-200 text-slate-500 hover:border-slate-300'
                    } disabled:opacity-50`}
                  >
                    {ROLE_LABELS[r]}
            ...
            {formRole === UserRole.SISWA && (
              <>
                <Field label="Nomor Induk Siswa (NIS)" error={formErrors.nis}>
                  <input type="text" placeholder="Contoh: 26051" value={formNis} onChange={e => setFormNis(e.target.value)} className="input text-xs py-2 font-mono" />
                </Field>
                <Field label="Kelas" error={formErrors.kelas}>
                  <input type="text" placeholder="Contoh: XI RPL 1" value={formKelas} onChange={e => setFormKelas(e.target.value)} className="input text-xs py-2 uppercase" />
                </Field>
              </>
            )}
            ...
            {formRole === UserRole.WALI_KELAS && (
              <>
                <Field label="NIP" error={formErrors.nip}>
                  <input type="text" placeholder="Contoh: 198205..." value={formNip} onChange={e => setFormNip(e.target.value)} className="input text-xs py-2 font-mono" />
                </Field>
            ...
            {formRole !== UserRole.SISWA && formRole !== UserRole.WALI_KELAS && (
              <div className="col-span-2">
                <Field label="NIP" error={formErrors.nip}>
                  <input type="text" placeholder="Masukkan NIP Pegawai..." value={formNip} onChange={e => setFormNip(e.target.value)} className="input text-xs py-2 font-mono" />
                </Field>
              </div>
            )}
          </div>
          ...
          <div className="flex gap-2.5 pt-4 border-t border-slate-100 justify-end">
            <button type="button" onClick={onClose} className="px-4 py-2 rounded-xl border border-slate-200 bg-white text-xs font-bold text-slate-500 hover:bg-slate-50">
              Batal
            </button>
            <button type="submit" className="px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 shadow-md shadow-blue-500/10 text-white font-bold text-xs">
