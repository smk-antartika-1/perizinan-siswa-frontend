...
export default function RekapPage() {
  ...
  const SUMMARY_STATS = [
    { label: 'Total Semua Izin', value: stats.total, icon: BarChart3, color: 'bg-blue-500' },
    { label: 'Sedang Keluar', value: stats.active, icon: TrendingUp, color: 'bg-emerald-500' },
    { label: 'Menunggu Proses', value: stats.pending + stats.approvedWali, icon: Clock, color: 'bg-amber-500' },
    { label: 'Selesai', value: stats.completed, icon: CheckCircle, color: 'bg-slate-500' },
  ];
      ...
      <div className="page-header">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="page-subtitle">Rekapitulasi lengkap data perizinan siswa</p>
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {SUMMARY_STATS.map(s => (
          <div key={s.label} className="stat-card">
            <div className={`w-11 h-11 rounded-xl ${s.color} flex items-center justify-center text-white shadow-lg flex-shrink-0`}>
              <s.icon size={20} />
            </div>
            <div>
              ...
              <div key={kelas} className="card-lg p-5 space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">{items.length} perizinan tercatat</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    <div key={x.label} className="bg-slate-50 rounded-xl p-2.5">
                      <p className="text-[10px] text-slate-400 font-bold uppercase">{x.label}</p>
                ...
                <div className="space-y-1.5">
                  {items.slice(0, 2).map(p => (
                    <div key={p.id} className="flex items-center justify-between text-xs py-1">
                      <span className="text-slate-600 truncate flex-1 mr-2">{p.studentName}</span>
