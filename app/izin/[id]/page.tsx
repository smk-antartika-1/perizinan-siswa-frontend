...
import { 
  ChevronLeft, Calendar, Clock, User, Shield, FileText, 
  CheckCircle2, AlertCircle, Printer, MessageSquare, History, Check, X, ShieldCheck
} from 'lucide-react';
...
export default function PermissionDetailPage() {
    ...
    return (
      <AppShell>
        <div className="flex flex-col items-center justify-center py-20 gap-3 text-slate-400">
          ...
          <h2 className="text-lg font-bold text-slate-800">Detail Perizinan Tidak Ditemukan</h2>
          <p className="text-sm">Maaf, izin dengan nomor ID {id} tidak terdaftar di sistem.</p>
  ...
  const studentUser = users.find(u => u.id === permission.studentId || u.name === permission.studentName);
  ...
  const handleApprove = () => {
    ...
    let nextStatus = permission.status;
    let auditAction = '';
    let updateFields: Partial<typeof permission> = {};

    if (permission.status === PermissionStatus.PENDING) {
      nextStatus = PermissionStatus.APPROVED_WALI;
      auditAction = 'Disetujui Wali Kelas';
      updateFields = {
    ...
    const newAudit = [
      ...(permission.auditLog || []),
      {
        ...
        timestamp: nowStr,
        details: commentText ? `Catatan: "${commentText}"` : undefined,
      }
    ];

    // Add Comment
    const newComments = [...(permission.comments || [])];
    if (commentText) {
      newComments.push({
  ...
  const handleReject = () => {
    ...
    const newAudit = [
      ...(permission.auditLog || []),
      {
        ...
        actorRole: currentUser.role,
        timestamp: nowStr,
        details: `Alasan Penolakan: "${commentText}"`,
      }
    ];
    ...
    newComments.push({
      ...
      userName: currentUser.name,
      userRole: currentUser.role,
      text: `Ditolak dengan alasan: ${commentText}`,
      timestamp: nowStr,
    });

    updatePermission(permission.id, {
      status: PermissionStatus.REJECTED,
      rejectedReason: commentText,
      comments: newComments,
      auditLog: newAudit,
    });
  ...
  const steps = [
    ...
    { 
      label: permission.approvedByWaliName ? `Wali Kelas (${permission.approvedByWaliName.split(' ')[0]})` : 'Wali Kelas', 
      desc: permission.approvedByWaliName ? `Disetujui: ${permission.approvedByWaliName}` : 'Menunggu Wali Kelas',
      done: permission.status !== PermissionStatus.PENDING && permission.status !== PermissionStatus.REJECTED,
      active: permission.status === PermissionStatus.PENDING,
      rejected: permission.status === PermissionStatus.REJECTED && !permission.approvedByWaliId,
      timestamp: permission.approvedByWaliId ? (permission.auditLog?.find(a => a.action.includes('Wali'))?.timestamp) : null
    },
    { 
      label: permission.approvedByPiketName ? `Guru Piket (${permission.approvedByPiketName.split(' ')[0]})` : 'Guru Piket / QR', 
      desc: permission.approvedByPiketName ? `Dicetak: ${permission.approvedByPiketName}` : 'Menunggu Guru Piket',
      done: permission.status === PermissionStatus.APPROVED_PIKET || permission.status === PermissionStatus.COMPLETED,
      active: permission.status === PermissionStatus.APPROVED_WALI,
      rejected: permission.status === PermissionStatus.REJECTED && !!permission.approvedByWaliId,
      timestamp: permission.approvedByPiketId ? (permission.auditLog?.find(a => a.action.includes('Piket'))?.timestamp) : null
    },
    { 
      label: 'Kembali', 
      desc: permission.category === 'sakit' ? 'Sakit (Bebas Kepulangan)' : (permission.actualReturnTime ? `Tiba: ${formatTime(permission.actualReturnTime)}` : 'Siswa Belum Kembali'),
      done: permission.status === PermissionStatus.COMPLETED || permission.category === 'sakit',
      active: permission.status === PermissionStatus.APPROVED_PIKET && permission.category !== 'sakit',
  ...
  const canApprove = () => {
    ...
    if (permission.status === PermissionStatus.PENDING && currentUser.role === UserRole.WALI_KELAS) return true;
    if (permission.status === PermissionStatus.APPROVED_WALI && currentUser.role === UserRole.GURU_PIKET) return true;
    return false;
  };

  const isPiketOrAdmin = currentUser && (currentUser.role === UserRole.GURU_PIKET || currentUser.role === UserRole.ADMIN || currentUser.role === UserRole.SECURITY);
      ...
      <div className="page-header flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {isPiketOrAdmin && (
          <div className="flex flex-wrap items-center gap-3">
            ...
            {(permission.status === PermissionStatus.APPROVED_PIKET || permission.status === PermissionStatus.COMPLETED) && (
              <button 
                onClick={handlePrint}
            ...
            {permission.category === 'sakit' ? (
              ...
              (permission.status === PermissionStatus.APPROVED_PIKET || permission.status === PermissionStatus.COMPLETED) && (
                <label className="flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 cursor-pointer transition-colors shadow-sm select-none">
                  ...
                  />
                  <span className="text-xs font-bold text-slate-700">Sudah Kembali ke Sekolah</span>
                </label>
              )
            )}
          </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="card p-6 border-slate-200">
            <div className="flex items-center gap-2 pb-4 mb-4 border-b border-slate-100">
              ...
              <h3 className="font-bold text-slate-800 text-sm">Informasi Pengajuan & Siswa</h3>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div>
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">Siswa Bersangkutan</span>
                  <button 
                  ...
                  <span className="text-xs text-slate-500 block">NIS: {studentNis} | Kelas: {permission.kelas}</span>
                </div>

                <div>
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">Kategori Keperluan</span>
                  <span className={`inline-block px-2 py-0.5 rounded-lg text-[10px] font-bold uppercase tracking-wider mt-1 ${categoryStyles[permission.category || 'lainnya']}`}>
                    {categoryLabels[permission.category || 'lainnya']}
                  </span>
                </div>

                <div>
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">Alasan / Keterangan</span>
                  <p className="text-xs font-medium text-slate-700 mt-1 italic">"{permission.reason}"</p>
                </div>
              </div>

              <div className="space-y-4">
                <div>
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">Waktu Berangkat</span>
                  <div className="flex items-center gap-2 mt-1 text-slate-600">
                    <Calendar size={13} className="text-slate-400" />
                    <span className="text-xs font-medium">{formatDate(permission.departureTime)}</span>
                    <Clock size={13} className="text-slate-400 ml-1" />
                    <span className="text-xs font-medium">{formatTime(permission.departureTime)} WIB</span>
                  </div>
                </div>

                <div>
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">Estimasi Kembali</span>
                  <div className="flex items-center gap-2 mt-1 text-slate-600">
                    <Calendar size={13} className="text-slate-400" />
                    <span className="text-xs font-medium">{formatDate(permission.estimatedReturnTime)}</span>
                    <Clock size={13} className="text-slate-400 ml-1" />
                    <span className="text-xs font-medium">
                      {permission.category === 'sakit' ? 'Selesai KBM (Bebas)' : `${formatTime(permission.estimatedReturnTime)} WIB`}
                    </span>
                  </div>
                </div>
                ...
                {permission.nomorPolisi && (
                  <div>
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">No. Polisi Kendaraan</span>
          ...
          <div className="card p-6 border-slate-200">
            <div className="flex items-center gap-2 pb-4 mb-6 border-b border-slate-100">
              ...
              <h3 className="font-bold text-slate-800 text-sm">Perkembangan Persetujuan</h3>
            </div>
            ...
            <div className="relative pl-6 md:pl-0 md:flex md:items-start justify-between gap-6 space-y-6 md:space-y-0">
              ...
              {steps.map((step, idx) => {
                const isStepRejected = step.rejected || (permission.status === PermissionStatus.REJECTED && idx === 2 && !permission.approvedByWaliId);
                return (
                  <div key={idx} className="relative md:flex-1 flex md:flex-col items-start md:items-center text-left md:text-center group">
                    {/* Stepper Dot */}
                    ...
                    {idx < steps.length - 1 && (
                      <div className={`hidden md:block absolute top-3 left-[calc(50%+12px)] right-[calc(-50%+12px)] h-0.5 transition-colors ${
                        steps[idx+1].done || (steps[idx+1].isSakit && step.done) ? 'bg-emerald-500' : 'bg-slate-200'
                      }`} />
                    )}
                    ...
                    <div className="ml-4 md:ml-0 md:mt-3 flex-1">
                      <p className={`text-xs font-bold leading-tight ${
                        isStepRejected ? 'text-red-600' : step.done ? 'text-slate-800' : 'text-slate-400'
                      }`}>{step.label}</p>
                      <p className="text-[10px] text-slate-400 font-semibold mt-0.5">{step.desc}</p>
                      {step.timestamp && (
                        <p className="text-[9px] text-slate-350 font-mono mt-1">{formatDateTime(step.timestamp)}</p>
                      )}
                    </div>
                  </div>
                );
          ...
          <div className="card p-6 border-slate-200">
            <div className="flex items-center gap-2 pb-4 mb-4 border-b border-slate-100">
              <History size={16} className="text-blue-600" />
              <h3 className="font-bold text-slate-800 text-sm">Riwayat Aktivitas & Audit Trail</h3>
            </div>
            ...
            <div className="space-y-4">
              {permission.auditLog && permission.auditLog.length > 0 ? (
                ...
                <p className="text-slate-400 text-xs italic">Belum ada riwayat aktivitas yang tercatat</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-8">
          ...
          {canApprove() && (
            <div className="card p-6 border-blue-200 bg-gradient-to-br from-white to-blue-50/20">
              <div className="flex items-center gap-2 pb-4 mb-4 border-b border-slate-100">
                ...
                <h3 className="font-bold text-slate-800 text-sm">Formulir Tindakan Persetujuan</h3>
              </div>

              <div className="space-y-4">
                <p className="text-xs text-slate-500 leading-relaxed">
                  Sebagai <strong>{currentUser?.name}</strong> ({currentUser?.role === UserRole.WALI_KELAS ? 'Wali Kelas' : 'Guru Piket'}), Anda berhak menindaklanjuti pengajuan perizinan ini.
                </p>

                {/* Additional inputs based on role */}
                {permission.status === PermissionStatus.APPROVED_WALI && currentUser?.role === UserRole.GURU_PIKET && (
                  <div>
                    <label className="label text-[10px] uppercase font-bold text-slate-500 tracking-wider">No. Polisi Kendaraan (Opsional)</label>
                    <input
                      type="text"
                      placeholder="Contoh: L 1234 ABC"
                      value={nomorPolisiInput}
                      onChange={e => setNomorPolisiInput(e.target.value.toUpperCase())}
                ...
                {/* Comments Box */}
                <div>
                  <label className="label text-[10px] uppercase font-bold text-slate-500 tracking-wider">Catatan Tambahan / Alasan</label>
                  <textarea
                    rows={3}
                    placeholder={permission.status === PermissionStatus.PENDING ? "Tulis catatan persetujuan atau alasan penolakan..." : "Masukkan catatan (opsional)..."}
                    value={commentText}
          ...
          {(permission.status === PermissionStatus.APPROVED_PIKET || permission.status === PermissionStatus.COMPLETED) && (
            <div className="card p-6 border-slate-200 bg-slate-50">
              <div className="flex items-center gap-2 pb-4 mb-4 border-b border-slate-200">
                ...
                <h3 className="font-bold text-slate-800 text-sm">E-Ticket QR Perizinan</h3>
              </div>

              <div className="flex flex-col items-center p-4 bg-white border border-slate-200 rounded-3xl shadow-sm text-center">
                <p className="font-mono text-xs font-bold text-slate-400">ID TIKET: {permission.id}</p>
                
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 my-4">
                  <QRCodeSVG value={generateQRValue(permission)} size={140} level="H" />
                </div>

                <h4 className="font-bold text-slate-800 text-sm">{permission.studentName}</h4>
                <p className="text-xs text-slate-400 font-semibold mt-0.5">{permission.kelas}</p>
                <p className="text-[10px] text-blue-600 font-semibold uppercase tracking-wider mt-3 bg-blue-50 border border-blue-100 px-2.5 py-1 rounded-full">
                  {permission.status === PermissionStatus.COMPLETED ? 'Tiket Expired (Selesai)' : 'Tiket Aktif (Scan Gate)'}
                </p>
          ...
          <div className="card p-6 border-slate-200">
            <div className="flex items-center gap-2 pb-4 mb-4 border-b border-slate-100">
              ...
              <h3 className="font-bold text-slate-800 text-sm">Catatan & Diskusi ({permission.comments?.length || 0})</h3>
            </div>

            <div className="space-y-4 max-h-60 overflow-y-auto pr-1">
              {permission.comments && permission.comments.length > 0 ? (
                permission.comments.map((comment) => (
                  <div key={comment.id} className="p-3 bg-slate-50 border border-slate-100 rounded-2xl">
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[11px] font-bold text-slate-700 truncate max-w-[120px]">{comment.userName}</span>
                        <span className="text-[9px] text-blue-600 bg-blue-50 px-1 rounded font-medium">{comment.userRole}</span>
                      </div>
                      <span className="text-[9px] text-slate-350 font-mono">{formatDateTime(comment.timestamp)}</span>
                    </div>
                    <p className="text-xs text-slate-600 mt-2 italic">"{comment.text}"</p>
                  </div>
                ))
              ) : (
                <p className="text-slate-400 text-xs italic text-center py-6">Belum ada catatan yang ditulis</p>
              )}
            </div>
              ...
              <div className="mt-4 pt-4 border-t border-slate-100 space-y-2">
                ...
                <button
                  onClick={() => {
                    ...
                    updatePermission(permission.id, { comments: newComments });
